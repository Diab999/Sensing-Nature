@php
    $settings = \App\Models\AppSetting::query()->first();
    $rawPhone = $settings?->whatsapp_phone ?: env('WHATSAPP_PHONE');
    $whatsappPhone = $rawPhone ? preg_replace('/\D+/', '', $rawPhone) : '';
@endphp

@if($whatsappPhone)
<div class="wh-api">
    <div class="wh-fixed whatsapp-pulse">
        <!-- WhatsApp Link with Fallback -->
        <a href="https://wa.me/{{ $whatsappPhone }}" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" class="whatsapp-link">
            <button class="wh-ap-btn" type="button" title="{{ __('messages.chat_with_us') }}">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.488"/>
                </svg>
            </button>
        </a>
        
        <!-- Alternative Contact Methods (Hidden by default, shown on hover) -->
        <div class="contact-alternatives">
            <div class="contact-option">
                <a href="tel:{{ $whatsappPhone }}" class="contact-link phone-link" title="Call us">
                    <i class="bi bi-telephone-fill"></i>
                </a>
            </div>
            <div class="contact-option">
                <a href="mailto:info@sensingnature.com" class="contact-link email-link" title="Email us">
                    <i class="bi bi-envelope-fill"></i>
                </a>
            </div>
        </div>
    </div>
</div>

<style>
.wh-api {
    position: fixed;
    bottom: 20px;
    right: 20px;
    z-index: 9999;
    pointer-events: auto;
}

.wh-fixed {
    position: relative;
}

.wh-fixed > a {
    display: block;
    text-decoration: none;
}

.wh-ap-btn {
    outline: none;
    width: 60px;
    height: 60px;
    border: 0;
    background-color: #25D366;
    padding: 0;
    border-radius: 50%;
    box-shadow: 0 4px 12px rgba(37, 211, 102, 0.4);
    cursor: pointer;
    transition: all 0.3s ease;
    position: relative;
    display: flex;
    align-items: center;
    justify-content: center;
    color: white;
}

.wh-ap-btn:hover {
    background-color: #128C7E;
    transform: scale(1.1);
    box-shadow: 0 6px 20px rgba(37, 211, 102, 0.6);
}

.wh-ap-btn svg {
    width: 32px;
    height: 32px;
}

.wh-ap-btn::before {
    content: "{{ __('messages.chat_with_us') }}";
    display: block;
    position: absolute;
    right: 70px;
    top: 50%;
    transform: translateY(-50%);
    height: 35px;
    background: #075E54;
    color: #fff;
    font-weight: 500;
    font-size: 14px;
    border-radius: 20px;
    width: 0;
    opacity: 0;
    padding: 0;
    transition: all 0.3s ease;
    line-height: 35px;
    text-align: center;
    white-space: nowrap;
    overflow: hidden;
}

.wh-fixed:hover .wh-ap-btn::before {
    opacity: 1;
    width: 120px;
    padding: 0 15px;
}

/* Contact Alternatives */
.contact-alternatives {
    position: absolute;
    bottom: 70px;
    right: 0;
    display: flex;
    flex-direction: column;
    gap: 10px;
    opacity: 0;
    transform: translateY(10px);
    transition: all 0.3s ease;
    pointer-events: none;
}

.wh-fixed:hover .contact-alternatives {
    opacity: 1;
    transform: translateY(0);
    pointer-events: auto;
}

.contact-option {
    display: flex;
    justify-content: center;
}

.contact-link {
    width: 45px;
    height: 45px;
    border-radius: 50%;
    display: flex;
    align-items: center;
    justify-content: center;
    color: white;
    text-decoration: none;
    transition: all 0.3s ease;
    font-size: 18px;
}

.phone-link {
    background-color: #007bff;
}

.phone-link:hover {
    background-color: #0056b3;
    transform: scale(1.1);
    color: white;
}

.email-link {
    background-color: #dc3545;
}

.email-link:hover {
    background-color: #c82333;
    transform: scale(1.1);
    color: white;
}

.whatsapp-pulse {
    position: relative;
    animation: pulse 2s infinite;
}

@keyframes pulse {
    0% {
        box-shadow: 0 0 0 0 rgba(37, 211, 102, 0.7);
    }
    70% {
        box-shadow: 0 0 0 10px rgba(37, 211, 102, 0);
    }
    100% {
        box-shadow: 0 0 0 0 rgba(37, 211, 102, 0);
    }
}

/* RTL support */
[dir="rtl"] .wh-ap-btn::before {
    right: auto;
    left: 70px;
}

[dir="rtl"] .contact-alternatives {
    right: auto;
    left: 0;
}

/* Responsive adjustments */
@media (max-width: 768px) {
    .wh-api {
        bottom: 15px;
        right: 15px;
    }
    
    .wh-ap-btn {
        width: 55px;
        height: 55px;
    }
    
    .contact-alternatives {
        bottom: 65px;
    }
    
    .contact-link {
        width: 40px;
        height: 40px;
        font-size: 16px;
    }
}
</style>

<script>
// Add error handling for WhatsApp links
document.addEventListener('DOMContentLoaded', function() {
    const whatsappLinks = document.querySelectorAll('.whatsapp-link');
    
    whatsappLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            // Try to open WhatsApp, if it fails, show alternative options
            try {
                // The link will open in a new tab, so we don't need to prevent default
                console.log('Opening WhatsApp with phone:', '{{ $whatsappPhone }}');
            } catch (error) {
                console.error('WhatsApp link error:', error);
                // Could add fallback behavior here if needed
            }
        });
    });
});
</script>
@endif


